...
import type { AlertsOutputItem, MetricsOutput } from './lib/types'
...
async function run() {
  ...
  if ((metrics?.week?.revenue_wow_pct ?? 0) < -10) {
    alerts.push({
      severity: 'medium',
      type: 'revenue',
      title: 'Revenue trend softened week-over-week',
      detail: `Revenue is down ${Math.abs(metrics?.week?.revenue_wow_pct ?? 0).toFixed(1)}% week-over-week.`
    })
  }
