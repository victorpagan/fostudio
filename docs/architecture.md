...
- Outside daytime lab hours, access activation sends an explicit Abode disarm at the 30-minute booking lead and verifies the alarm reports `disarmed` before the job succeeds. The Yale unlock automation is an unconditional direct-disarm fallback, not the primary disarm path. Alarm service calls poll the entity after Home Assistant accepts the command, and an end-of-window arm is skipped while any other booking access window is active.
