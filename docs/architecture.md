...
- Outside daytime lab hours, access activation disarms Abode at the 30-minute booking lead and verifies the alarm reaches `disarmed` before the job succeeds. The Yale unlock callback remains a fallback, not the primary disarm path. Arm/disarm service calls poll the alarm entity after Home Assistant accepts the command, and an end-of-window arm is skipped while any other booking access window is active.
- Abode actions read the current alarm state first and skip redundant arm/disarm calls, preventing an already-satisfied state from creating a false automation incident.
