...
- `fomailer` for mail sends; SendGrid for template/admin checks. FO Studio sends registry-backed lifecycle events including `account.signup`, `booking.memberCreated`, booking changes, the idempotent `booking.endingSoonReminder` extension CTA, membership changes, credits/holds top-ups, admin member charge receipts, contact, and broadcasts.
...
- Studio core writes: `bookings`, `booking_holds`, `calendar_blocks`, `customers`, `memberships`, `membership_tiers`, `membership_credit_grants`, `membership_checkout_sessions`, `credit_balance`, `credit_topup_sessions`, `hold_balance`, `hold_topup_sessions`, `credits_ledger`, `credit_ledger_operations`, `credit_ledger_repair_audit`, `hold_ledger`.
...
- Uses RPCs `get_secret`, `create_confirmed_booking_with_burn_and_rate`, `create_confirmed_booking_with_burn_no_membership_and_rate`, `reschedule_booking_with_credit_adjustment`, and `cancel_booking_with_credit_refund`. Booking creation, reschedule accounting, cancellation refunds, rate kind, and rate-policy snapshots cross only `service_role` boundaries.
...
- Supabase `pg_cron`/`pg_net` calls `POST /api/internal/access/process` every minute with `x-access-key`. The database scheduler is authoritative so a Home Assistant outage cannot also remove queue scheduling; the former HA automation is a disabled emergency fallback. The same durable queue delivers booking-ending reminders even when physical lock sync is disabled, while lock and alarm jobs remain paused.
...
- Access jobs activate 30 minutes before booking start and deactivate 30 minutes after booking end. Eligible account-owned, non-standby bookings also queue a Fomailer extension CTA 15 minutes before end. The message is idempotent per booking/end-time and revalidates membership/guest extension policy immediately before delivery.
...
- Before 11 AM and from 9 PM onward, access activation sends an explicit Abode disarm at the 30-minute booking lead and verifies the alarm reports `disarmed` before the job succeeds. From 7 PM through 8:59 PM, booking and lock events leave the shared employee-controlled Abode state untouched. The Yale unlock automation is an unconditional direct-disarm fallback, not the primary disarm path. Door-code deactivation never changes Abode state. Bookings ending before 11 AM or at/after 9 PM queue `armed_away` for one hour after the saved booking end; the job is invalidated by an extension and is skipped while any other booking access window is active. Booking ends from 7 PM through 8:59 PM schedule no alarm action. FO Studio does not own the separate 9 PM `armed_home` facility policy.
- Abode is a shared-control system: employees and external schedules may arm or disarm the physical panel independently of FO Studio. Every booking-boundary arm or disarm therefore sends an explicit reconciliation command instead of treating Home Assistant's last observed state as authoritative.
...
- Credit accounting consumes earliest-expiring lots and writes every automated booking, reschedule, and paid-hold debit with the source lot's `expires_at`. Booking reductions and cancellations restore that exact debit expiration instead of creating permanent refund credits. `credit_ledger_operations` provides idempotency, and the migration-scoped `credit_ledger_repair_audit` preserves before-state evidence for historical rows that required expiry reclassification or splitting. The ledger lifetime sum is invariant across that repair. Database migrations `20260906033152_fostudio_credit_lot_accounting.sql` and `20260906040828_index_fostudio_credit_repair_audit_ledger.sql` are `Ready` in production as of 2026-09-06.
...
- Booking hardening rollout order is additive RPC migration, application deployment, then Data API RLS lockdown and calendar cron activation. Credit-lot accounting likewise requires migrations `20260906033152_fostudio_credit_lot_accounting.sql` and `20260906040828_index_fostudio_credit_repair_audit_ledger.sql` before deploying its reschedule/cancel API consumers. Reversing that order can temporarily break booking mutations. Scheduled external access followed the required additive `booking_external_access` migration-first order before its admin UI/API deployment; calendar reads retain a missing-table compatibility fallback for older environments.
