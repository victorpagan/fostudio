-- =============================================================================
-- Booking burn allocation: consume expiring credits first
-- - Replays active credit ledger into temporary lots
-- - New booking burns are split across those lots by earliest expiry
-- - Burn entries inherit source lot expiry so expired lots/net burns age out together
-- =============================================================================
...
BEGIN
  ...
  WHILE v_remaining_burn > 0 LOOP
    ...
    IF NOT FOUND THEN
      ...
      VALUES (p_user_id, v_membership_id, -v_remaining_burn, 'booking_burn', v_booking_id::text);
      v_balance_debt := v_balance_debt + v_remaining_burn;
      v_remaining_burn := 0;
      EXIT;
    END IF;
