-- =============================================================================
-- Handle cadence swaps where old/new grants share period_start + due_at
-- - daily -> monthly can collide on same membership_id + billing_period_start + due_at
-- - include billing_period_end in unique key / upsert target to avoid blocking
-- =============================================================================
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.membership_credit_grants
      DROP CONSTRAINT membership_credit_grants_unique_period_due;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'membership_credit_grants_unique_period_due_end'
      AND conrelid = 'public.membership_credit_grants'::regclass
  ) THEN
    ALTER TABLE public.membership_credit_grants
      ADD CONSTRAINT membership_credit_grants_unique_period_due_end
      UNIQUE (membership_id, billing_period_start, billing_period_end, due_at);
  END IF;
END $$;
