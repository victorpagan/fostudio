-- Refresh registry copy with the vetted FO Studio template proposal.
-- This intentionally updates saved registry copy so deployed environments load the new wording.
WITH updated_copy AS (
  SELECT *
...
UPDATE public.mail_template_registry AS registry
SET
  subject_template = updated_copy."subjectTemplate",
  preheader_template = updated_copy."preheaderTemplate",
  body_template = updated_copy."bodyTemplate",
  updated_at = now()
FROM updated_copy
WHERE registry.event_type = updated_copy.event_type;
