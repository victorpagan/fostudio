-- =============================================================================
-- Guard against duplicate processed grants for the same membership + due_at
-- - We intentionally allow multiple scheduled rows across period rewrites, but
--   only one row should ever be processed for a specific due timestamp.
-- - This prevents accidental double-mint when legacy duplicate rows exist.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    -- Serialize grant processing per user across workers.
    PERFORM pg_advisory_xact_lock(hashtext(v_grant.user_id::text));

    -- Dedupe guard: never process a second row for same membership + due_at.
    IF EXISTS (
      SELECT 1
      FROM public.membership_credit_grants existing
      WHERE existing.membership_id = v_grant.membership_id
        AND existing.due_at = v_grant.due_at
        AND existing.status = 'processed'
        AND existing.id <> v_grant.id
    ) THEN
      UPDATE public.membership_credit_grants
         SET status = 'canceled',
             processed_credits = 0,
             processed_at = now(),
             last_error = 'duplicate_due_processed'
       WHERE id = v_grant.id;
