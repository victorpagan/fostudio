-- =============================================================================
-- Prevent cadence-swap grant collisions at identical due_at timestamps
-- - Old uniqueness (membership_id, due_at) can block new-period grants after swap
--   when an already-processed prior grant shares the same due_at.
-- - Use period-scoped uniqueness and matching ON CONFLICT target.
-- =============================================================================
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.membership_credit_grants
      DROP CONSTRAINT membership_credit_grants_unique_due;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'membership_credit_grants_unique_period_due'
      AND conrelid = 'public.membership_credit_grants'::regclass
  ) THEN
    ALTER TABLE public.membership_credit_grants
      ADD CONSTRAINT membership_credit_grants_unique_period_due
      UNIQUE (membership_id, billing_period_start, due_at);
  END IF;
END $$;
...
BEGIN
  ...
  WHILE v_due_at < p_period_end LOOP
    ...
    ON CONFLICT (membership_id, billing_period_start, due_at)
    DO UPDATE
